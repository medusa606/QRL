...
def updateWeights(features,q_vals_future,q_argmax,feat_weights,reward,alpha,current_qval):
	...
	for agentID in range(0, nA):
		...
		for wi in range(0, numFeat):
			...
			old_weight = (float)(feat_weights[0,wi])			
			...
			new_weight = old_weight + alpha * difference * features[agentID,wi]
			feat_weights[0,wi] = new_weight
			# print("********************************")
			# print("***      Weight Update       ***")
			# print("********************************")						
			# print("agent = ", agentID)
...
nTests = 100000					# Number of experiements to run
...
reward_no_test_gen = -100		# penalty for failing to generate test before AV exits
...
if loopAgentList:
	...
	epsList = [0.02,0.05,0.1,0.15,0.2,0.25,0.3]
else:
	nAList = [nA]
...
# for epsilon_fixed in epsList:
	...
	feat_weights = np.array([np.random.rand(nF)])
	# feat_weights = np.array([np.random.rand(nF)],1)
	...
	while(not(done)) and (nExp <= nTests-1):
		...
		if plotWeights:
			x = np.arange(len(feat_weights[0,:]))
			# x = np.arange(len(feat_weights))
			if Wcounter==1: 
				y=feat_weights
				Wcounter=Wcounter+1
			else: 
				temp = np.transpose(feat_weights[0,:])
				# temp = np.transpose(feat_weights)
				y = np.vstack((y,temp))
				Wcounter=Wcounter+1
		...
		if plotQvalues:
			...
			if Qcounter==1: 
				...
				y = np.vstack((y,temp))
				Qcounter=Qcounter+1
			if (Qcounter>2) and ((nExp>nTests-2) or (nExp%display_chart_modulo==0)):
				plt.plot(y, label=tags)
				plt.title('Q-values') 
				Qfig.canvas.draw()
				Qimg = np.fromstring(Qfig.canvas.tostring_rgb(), dtype=np.uint8,	sep='')
